			...
			if(node->getName() == "CoteDroit"){
				if(modele->getEcritureEnCours()){
					...
					case GUIEventAdapter::KEY_F:
						{ 
							modele->changeCoeffficient();
							modele->setPlateauAJour(false);
							break;
						}
					...
					default:
						{
							if(!modele->getJeuFinis()){
								...
								case GUIEventAdapter::KEY_Up:
									{ 
										modele->deplacerSelection(EV::EventValeur::HAUT);
										break;
									}
								case GUIEventAdapter::KEY_Down:
									{ 

										modele->deplacerSelection(EV::EventValeur::BAS);
										break;
									}
								...
								case GUIEventAdapter::KEY_Right:
									{ 
										...
										modele->deplacerSelection(EV::EventValeur::DROITE);
										break;
									}
								case GUIEventAdapter::KEY_Left:
									{ 

										modele->deplacerSelection(EV::EventValeur::GAUCHE);
										break;
									}
								}
...
void EvenementUtilisateur::changerAspectBouton(ref_ptr<GUIEventAdapter> ea, Node* node){
	...
	if(intersector->containsIntersections()){
		...
		if(y > 3.5 && y < 4.44){
			...
			modele->changerAspectBouton(BV::BoutonValeur::NB_AIDE);

		}
		else if(y > 5.5 && y < 6.44){
			// charger 2

			modele->changerAspectBouton(BV::BoutonValeur::NB_CHARGER);
			...
			modele->changerAspectBouton(BV::BoutonValeur::NB_SAUVER);

		}
		else if(y > 9.5 && y < 10.44){
			//redemarrer 1

			modele->changerAspectBouton(BV::BoutonValeur::NB_REDEMARRER);
		}


	}else{
		modele->changerAspectBouton(BV::BoutonValeur::NB_RIEN);
	}
